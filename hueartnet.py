...
class HueEntertainment:
    def __init__(self, ip, username, key, group_name=None, mapping=None):
        ...
        if mapping:
            for mp in mapping:
                self.mapping[mp["light"]] = (
                    mp["start"] - 1,
                    mp.get("fine", False),
                )  # start in dmx is from 1
        self.sock = None

        self.dtls_cli_ctx = tls.ClientContext(
    ...
    def _user_get(self, api, data=None):
        return requests.get(
            f"http://{self.ip}/api/{self.username}/{api}", json=data
        ).json()
...
if not "group" in cfg["hue"]:
    h = HueEntertainment(
        cfg["hue"]["ip"], cfg["hue"]["username"], cfg["hue"]["clientkey"]
    )
...
a = ArtNetReceiver(
    cfg["art-net"]["ip"], cfg["art-net"]["port"], cfg["art-net"]["universe"]
)
